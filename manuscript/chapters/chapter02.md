...
**Sections**: 6
**Figures**: 3 (1 manual diagram + 2 Mermaid graphs)
**Code Examples**: 10+

### Writing Focus
- **Target audience**: Programmers, compiler writers, assembly learners
- **Depth level**: Detailed but practical (focus on usage, not just specification)
- **Key concepts**: Registers, CSRs, privilege levels, calling convention, stack frames
- **Comparisons**: ARM64 vs MIPS (register usage, calling conventions)

### Key Takeaways for Readers
1. 32 general-purpose registers (x0-x31) with x0 hardwired to zero
2. CSRs provide controlled access to privileged state
3. Three privilege levels (M/S/U) with controlled transitions
4. Standard calling convention (8 argument registers, caller/callee-saved)
5. Stack frames follow standard structure (prologue/epilogue pattern)

### Technical Highlights
- Complete register file diagram (manual figure)
- CSR address space organization
- Privilege level state machine
- Function call examples with prologue/epilogue
- Comparison table: RISC-V vs ARM64 vs MIPS

### Pedagogical Approach
- Start with concrete (registers) before abstract (CSRs)
- Provide complete code examples for each concept
- Show both hardware view (x0-x31) and software view (ABI names)
- Explain "why" for conventions (caller/callee-saved rationale)
- Compare with familiar architectures

### Future Expansion (for v1.0+)
- More detailed CSR field descriptions
- Advanced calling convention scenarios (varargs, floating-point)
