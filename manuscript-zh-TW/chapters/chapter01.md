...
1. **理解 ISA 的角色**：明白指令集架構 (ISA) 作為軟硬體介面的核心地位
2. **掌握 RISC-V 核心哲學**：模組化、精簡、開源——這三大支柱如何影響設計決策
3. **區分商業模式差異**：理解 RISC-V 與 x86/ARM 在授權與生態系統上的根本不同
4. **解讀 ISA 命名**：能夠看懂 `RV64GC`、`RV32IMC` 等 ISA 字串的含義
5. **建立開發環境**：成功安裝 RISC-V 工具鏈並執行第一個程式

---

## 💡 情境引入：茶水間的抉擇

> **場景**：週一早晨，公司茶水間的咖啡機發出嗡嗡的運作聲。空氣中瀰漫著咖啡香，但小華的表情卻很苦澀。

**小華**：（嘆氣）「杰哥，PM 剛才丟了一個新專案給我，說這次 MCU 選型要評估用 RISC-V，不要用我們慣用的 ARM Cortex-M。我頭都痛了，這兩個到底差在哪？不都是跑 C 語言嗎？為什麼要自找麻煩？」

**阿杰**：（笑著接過剛泡好的咖啡）「別慌。這就像是你以前習慣吃『連鎖速食店套餐』，現在老闆想讓你試試『自助式私廚』。寫 C 語言雖然感覺差不多，但底層的遊戲規則變了。」

**小華**：「遊戲規則？你是說指令集架構？」

**阿杰**：「沒錯。你想想，我們用 ARM 的時候，雖然生態系很強，但那就是一家公司的產品。你要用它的 IP，公司就得付授權費 (License Fee)，而且它給你什麼功能，你就只能用什麼。如果我們想針對 AI 演算法做個特殊硬體加速，在 ARM 上能改嗎？」

**小華**：「根本不可能吧，原廠怎麼可能開放核心讓我們亂動。」

**阿杰**：「這就是 RISC-V 最大的價值。它是**開放標準 (Open Standard)**。就像 TCP/IP 或 Linux 一樣，沒有一家公司『擁有』它。如果我們需要特殊的加速功能，在 RISC-V 架構下，我們可以自己設計專屬指令加進去，不用看原廠臉色。」

**小華**：「聽起來很靈活，但這樣會不會很亂？沒有標準的話，程式跑得起來嗎？」

**阿杰**：「這是新手最常問的問題。RISC-V 聰明的地方在於它的**模組化 (Modular)** 設計。它有一個強制遵守的『基本款』，其他的擴充功能就像樂高積木一樣，想用再裝。這也是為什麼現在從 NVIDIA 到大學實驗室都在玩它的原因。」

**小華**：（眼神亮了起來）「聽起來有點意思......也就是說，我們不只是『使用者』，還能變成『設計者』？」

**阿杰**：「賓果！走吧，回座位我帶你裝一下環境，我們先從這個『樂高積木』的底座開始看起。」
...
## 🛠️ 實作練習：Lab 1.1 — Hello RISC-V World

現在讓我們動手實作！在這個 Lab 中，你將安裝 RISC-V 工具鏈並執行第一個程式。

### 實驗目標

1. 理解什麼是 Cross-Compiler（交叉編譯器）
2. 安裝 `riscv64-unknown-elf-gcc` 工具鏈
3. 成功編譯並模擬執行第一個 C 程式

### 環境準備

你可以選擇以下任一方式安裝 RISC-V 工具鏈：

**方式 A：使用 xPack 預編譯包（推薦新手）**

```bash
# 下載 xPack RISC-V GNU Toolchain
# https://github.com/xpack-dev-tools/riscv-none-elf-gcc-xpack/releases

# Linux/macOS 範例（請根據實際版本調整）
...
```bash
# 編譯（使用 Proxy Kernel 支援的 ELF 格式）
riscv64-unknown-elf-gcc -o hello hello.c

# 查看檔案格式
file hello
# 輸出應該類似：hello: ELF 64-bit LSB executable, UCB RISC-V, ...

# 使用 QEMU User Mode 執行（需要 qemu-riscv64）
qemu-riscv64 hello

# 或使用 Spike + Proxy Kernel 執行
spike pk hello
```
...
> 💭 **為什麼不能直接在你的電腦上執行 `./hello`？**
>
> 因為你的電腦 CPU 是 x86 或 ARM 架構，無法直接執行 RISC-V 的機器碼。這就是為什麼我們需要**模擬器** (QEMU/Spike)——它在你的 Host CPU 上模擬一個 RISC-V 處理器。
>
> 這也引出了一個重要概念：**Host（主機）** vs **Target（目標）**
>
> - Host：你正在使用的電腦（如 x86 Linux）
> - Target：你要編譯程式給的平台（如 RISC-V）
> - Cross-Compiler 的工作就是在 Host 上產生 Target 的程式碼

### 延伸挑戰（選做）

試著用 `objdump` 查看編譯出來的 assembly：
...
這些就是 RISC-V 的指令！在後續章節中，我們將學習如何閱讀這些指令。
