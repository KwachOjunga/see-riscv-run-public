...
**Control and Status Register 快速參考**
...
CSR address 為 12 bit，組織如下：

```
Bits [11:10]: Privilege Level
  00 = User/Unprivileged
  01 = Supervisor
  10 = Hypervisor（base spec 中保留）
  11 = Machine
...
- 從不足的 privilege level 訪問 CSR 會導致 illegal instruction exception
- 寫入 read-only CSR（bits [11:10] = 11）會導致 illegal instruction exception
- 未實現的 CSR 可能讀取為 zero 或導致 exception（implementation-defined）
...
| **mhartid** | 0xF14 | RO | Hardware thread ID（每個 hart 唯一） |
| **mconfigptr** | 0xF15 | RO | Pointer to configuration data structure |

**Usage**：這些 read-only CSR 識別 processor implementation。Software 可以使用它們來檢測 feature、應用 workaround 或報告 system information。
...
| **misa** | 0x301 | RW | ISA and extensions（可能是 read-only） |
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **mscratch** | 0x340 | RW | Scratch register for M-mode trap handler |
...
| **pmpcfg4-15** | 0x3A4-0x3AF | RW | PMP configuration register 4-15 |
| **pmpaddr0-15** | 0x3B0-0x3BF | RW | PMP address register 0-15 |
| **pmpaddr16-63** | 0x3C0-0x3EF | RW | PMP address register 16-63 |

**Note**：RV32 使用 pmpcfg0、pmpcfg2、pmpcfg4 等（僅 even-numbered）。RV64 使用 pmpcfg0、pmpcfg2、pmpcfg4 等，每個持有 8 個 configuration byte。
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **mcycle** | 0xB00 | RW | Machine cycle counter（lower 32/64 bit） |
| **minstret** | 0xB02 | RW | Machine instructions retired counter |
| **mhpmcounter3-31** | 0xB03-0xB1F | RW | Machine performance monitoring counter |
| **mcycleh** | 0xB80 | RW | Upper 32 bit of mcycle（RV32 only） |
| **minstreth** | 0xB82 | RW | Upper 32 bit of minstret（RV32 only） |
| **mhpmcounter3h-31h** | 0xB83-0xB9F | RW | Upper 32 bit of mhpmcounter（RV32 only） |
...
| **mhpmevent3-31** | 0x323-0x33F | RW | Machine performance monitoring event selector |

**Usage**：mcountinhibit 控制哪些 counter 是 active 的。設置 bit N 會停止 counter N 遞增，節省 power。
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **sstatus** | 0x100 | RW | Supervisor status register（mstatus 的 subset） |
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **sscratch** | 0x140 | RW | Scratch register for S-mode trap handler |
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **fflags** | 0x001 | RW | Floating-point accrued exception |
...
| CSR | Address | R/W | Description |
|-----|---------|-----|-------------|
| **cycle** | 0xC00 | RO | Cycle counter（lower 32/64 bit） |
| **time** | 0xC01 | RO | Timer（lower 32/64 bit） |
| **instret** | 0xC02 | RO | Instructions retired counter |
| **hpmcounter3-31** | 0xC03-0xC1F | RO | Performance monitoring counter |
| **cycleh** | 0xC80 | RO | Upper 32 bit of cycle（RV32 only） |
| **timeh** | 0xC81 | RO | Upper 32 bit of time（RV32 only） |
| **instreth** | 0xC82 | RO | Upper 32 bit of instret（RV32 only） |
| **hpmcounter3h-31h** | 0xC83-0xC9F | RO | Upper 32 bit of hpmcounter（RV32 only） |

**Note**：這些是 machine-level counter 的 read-only shadow。訪問可以被 mcounteren（for S-mode）或 scounteren（for U-mode）禁用。

---

## A.5 Debug CSRs

Debug CSR 僅在 Debug Mode 中可訪問（通過 debugger 或 trigger 進入）。
...
| **tinfo** | 0x7A4 | RO | Trigger info（supported type） |
| **tcontrol** | 0x7A5 | RW | Trigger control |
| **mcontext** | 0x7A8 | RW | Machine context register |
| **scontext** | 0x7AA | RW | Supervisor context register |

**Usage**：Trigger 啟用 hardware breakpoint 和 watchpoint。tselect 選擇要配置的 trigger，tdata1-3 配置選定的 trigger。
...
Bit  20: TVM（Trap Virtual Memory - trap SATP write）
...
Bits [1:0]: MODE
  0 = Direct（all trap to BASE）
  1 = Vectored（interrupt to BASE + 4*cause, exception to BASE）
```
...
Bits [59:44]: ASID（Address Space Identifier, 16 bit）
Bits [43:0]: PPN（Physical Page Number of root page table, 44 bit）
```
...
Bits [30:22]: ASID（9 bit）
Bits [21:0]: PPN（22 bit）
```
...
- 從較低 privilege 訪問 CSR → illegal instruction exception

**Read-Only Check**：

- CSR address bits [11:10] = 11 → read-only
- 寫入 read-only CSR → illegal instruction exception

**Implementation-Defined Behavior**：

- 未實現的 CSR 可能：
  - 讀取為 zero，寫入被忽略（WARL - Write Any, Read Legal）
  - 導致 illegal instruction exception
  - Implementation 必須記錄 behavior

---

## A.11 References

- **RISC-V Privileged Specification**：完整的 CSR definition 和 bit field
- **RISC-V Debug Specification**：Debug CSR（dcsr、dpc、tselect、tdata）
- **RISC-V ISA Manual**：CSR instruction 和 access rule
