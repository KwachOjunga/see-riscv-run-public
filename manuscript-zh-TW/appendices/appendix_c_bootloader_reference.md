...
**Purpose**: 最小的 ROM code，用於初始化 DRAM 並載入 FSBL。

**Constraints**：

- 必須適合小型 on-chip ROM（通常 16-64 KB）
- 初始時沒有 DRAM 可用
- 必須從 ROM 或 tightly-coupled memory（TCM）執行
...
**Purpose**: 從 storage 載入 second-stage bootloader（U-Boot）。

**Features**：

- Initialize storage controller（SPI、SD、eMMC）
- Load SSBL image from storage
- Verify SSBL（checksum 或 signature）
- Jump to SSBL
...
**Purpose**: 載入 kernel 和 device tree，設置 boot environment。
...
**Purpose**: 定義 bootloader 的 memory layout。
...
**Symptoms**: System 不 boot，沒有 output。

**Possible Causes**：

- Reset vector 指向無效 address
- ROM 未正確 map
- Clock 未初始化
...
**Symptoms**: System 在 DRAM init 後 hang，或 data corruption。

**Possible Causes**：

- 不正確的 DRAM controller configuration
